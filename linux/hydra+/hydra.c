...
/*************************/
	...
	int is_exec = hdr_is_self_executed() ;/*check for self executable mode*/
	if( is_exec == 1)
	{
      /*a self executed script! Load the internal embedded script */
	  PHDR_RAW_BUF rawbuff = hdr_load_stored_script() ;
	  if(rawbuff->len == 0)
	  {
		printf("***Fatal Error. The loading of the embedded script returned zero size.");
	    return ;
	  }
	  embbed_scr = rawbuff ; 
	  ...
	  if(argc == 3 )
	  {
		...
		if(is_exec == -1)
		{
		 printf("***Fatal Error. Error in is_exec -1\n");
	     return ;
		}
	else
	{
		...
		if(argv[1][0] != '-')
		{
		 ...
		 if((argv[1][0] == '-')&&(argv[1][1] == 'c'))
		 {
			...
			if (hydra == NULL)
			{
				printf("***FATAL ERROR : Hydra object did not created.\n");
				return 0 ;
			}
			main_script = argv[2] ;

			 /*setup the loader with the main script file*/
			 if(HydraLoadScript(hydra , main_script,NULL) == false)
			 {
		 ...
		 if((argv[1][0] == '-')&&(argv[1][1] == 'e'))
		 {
			...
			main_script = argv[2] ;
			 ...
			 hdr_create_executable(newf->stringa);
		 ...
		 {
		  printf("***Fatal Error : The flag [%s] is not a valid flag for the Hydra+ command line. valid flags : [-o,-c,-e]\n",argv[1]);
		  return 0 ;
		 }	
	 }
	...
	if (HydraLoadScript(hydra, main_script,embbed_scr) == false)
	{
